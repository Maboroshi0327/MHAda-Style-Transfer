...
SCENE = ["alley_2", "ambush_5", "bandage_2", "market_6", "temple_2"]
...
if __name__ == "__main__":
    ...
    for scene in SCENE:
        ...
        dataloader = DataLoader(
            Sintel(image_size=IMAGE_SIZE2, scene=scene, device=device),
            batch_size=1,
            shuffle=False,
            num_workers=4,
            prefetch_factor=2,
        ...
        maeMatrix = nn.L1Loss(reduction="none")
        ...
        for c1, c2, flow, mask in batch_iterator:
            with torch.no_grad():
                ...
                # MAE
                loss = torch.sum(mask * maeMatrix(cs2, warped_cs1)) / (cs1.shape[1] * cs1.shape[2] * cs1.shape[3])
        ...
        # Print final loss for the scene
        optical_loss = torch.sqrt(optical_loss) / count
        print(f"  Optical Flow Loss: {optical_loss}\n")

        # Store results
        results[scene] = optical_loss.item()

    # Print all results
    print("Final Results:")
    for scene, loss in results.items():
        print(f"  {scene}: {loss:.4f}")

    # Save results to a csv file
    with open("./results/sintel_results.csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Scene", "Optical Flow Loss"])
        for scene, loss in results.items():
            writer.writerow([scene, loss])
